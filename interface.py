...
def ok_button(formato):
    backup=var_backup.get()
    convert_image.main(filename, backup, formato)
...
button2 = Button(root,text="OK", command= lambda: ok_button(formato.get()))
button2.grid(row=3, column=2, pady=10)

var_backup = BooleanVar()
c1 = Checkbutton(root, text='Fazer backup', variable = var_backup, onvalue=True, offvalue=False, anchor=E)
c1.deselect()
